...
fn test_update_data_filters_reapplied() {
    ...
    assert_match!(socket.outgoing_rx.recv_timeout(std::time::Duration::from_secs(20)).unwrap(),
        CommMsg::Data(value) => {
            // Make sure it's a data update event.
            assert_match!(serde_json::from_value::<DataExplorerFrontendEvent>(value).unwrap(),
                DataExplorerFrontendEvent::DataUpdate
...
// While https://github.com/posit-dev/positron/issues/4222 is not fixed, we
// want to run data explorer tests sequentially to avoid random crashes.
#[test]
fn test_data_explorer() {
    r_test(|| {
        ...
        test_invalid_filters_preserved();
        test_data_explorer_special_values();
        test_export_data();
        test_update_data_filters_reapplied();
    })
}
